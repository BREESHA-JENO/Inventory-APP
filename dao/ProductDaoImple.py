...
class ProductDaoImplementation(ProductDaoService):
    ...
    def insert_products(self,product:Product)->bool:
        try:
            ...
            cursor.execute(self.INSERT_PRODUCT, (product.get_product_name(),product.get_unitprice(),product.get_category_id(),product.get_manufacture_date(),product.get_is_active()))
            self.conn.commit()
    ...
    def find_by_product_id(self,product_id:int):
        ...
        try:
            ...
            if row:
                product=Product(
                    productid=row["productid"],productName=row["productname"],unitprice=row["unitprice"],categoryid=row["categoryid"],manufacturedate=row["manufacturedate"],is_active=row["isActive"]
                )
        except Exception as e:
    ...
    def update_product(self,product:Product,product_id:int)->bool:
        try:
            cursor=self.conn.cursor(DictCursor)
            cursor.execute(self.UPDATE_PRODUCT,(product.get_product_name(),product.get_unitprice(),product_id))
            self.conn.commit()
            return cursor.rowcount==1
        except Exception as e:
    ...
    def disable_product(self,product:Product,product_id:int)->bool:
        try:
            cursor=self.conn.cursor(DictCursor)
            cursor.execute(self.DISABLE_PRODUCT,("N",product_id))
            self.conn.commit()
            return cursor.rowcount==1
        except Exception as e:
            print("Error disabling products:",e)
            return False
        finally:
            cursor.close()
